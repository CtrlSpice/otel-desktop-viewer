...
- **Orphans are swept, not cascaded.** Since no FK covers the arrays, the `Clear` and delete-by-id paths leave dictionary rows behind rather than reference-counting them. `ingest.SweepOrphans` builds the live id set by unnesting every owner and deletes what nothing references. It is driven entirely from retention, at two points: once before the first size measurement, so garbage from an earlier `Clear` can never be what tips the store over the cap, and again at the end of each prune round, since the prunes are what create orphans. The invariant both buy is that **no round deletes real telemetry to make room for rows nothing references** — which matters because retention is size-driven and orphans count toward the measured size.
