...
- **Empty span references stay signal-specific.** Optional parent, log, and exemplar span IDs are SQL NULL and JSON `null`. Link targets retain the existing required-string API shape: an empty OTLP linked span ID is stored as zero and rendered as `0000000000000000`, matching the frontend `LinkData.spanID: string` contract.
