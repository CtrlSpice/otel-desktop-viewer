<!-- LogsPage.svelte - Logs viewing page -->
<div class="flex flex-col items-center justify-center w-full overflow-y-auto">
  <div class="card bg-base-100 shadow-2xl my-16 min-w-[960px] p-8 w-4/5">
    <div class="card-header pb-0">
      <h1 class="text-2xl font-bold">Logs</h1>
      <div class="divider"></div>
    </div>

    <div class="card-body">
      <p class="text-base-content/80">
        This is where your logs will be displayed. Log entries, filtering, and
        search will go here.
      </p>

      <!-- Placeholder for logs content -->
      <div class="mt-8 p-8 bg-base-200 rounded-lg text-center">
        <p class="text-base-content/60">Logs content coming soon...</p>
      </div>
    </div>
  </div>
</div>
