...
/**
 * Assign colours from a stem-rotated pool (from `categoricalPalette`) to an
 * initial visible set. Walks `legendOrder` so slot-filling follows list order;
 * the first assigned key gets `pool[0]` (the metric type's stem colour).
 */
