...
// The extension instruments itself in both "enabled" and "self" -- the ingest
// distinction belongs to the exporter, not here.
func TestExtensionSelfTelemetry(t *testing.T) {
	assert.False(t, (&Config{}).SelfTelemetry())
	assert.False(t, (&Config{Telemetry: TelemetryDisabled}).SelfTelemetry())
	assert.True(t, (&Config{Telemetry: TelemetryEnabled}).SelfTelemetry())
	assert.True(t, (&Config{Telemetry: TelemetrySelf}).SelfTelemetry())
}
